...
if [ -f "action.yml" ]; then
    ...
    # Check inputs
    if grep -q "fail-on-severity:" action.yml && \
       grep -q "upload-sarif:" action.yml && \
       grep -q "working-directory:" action.yml; then
    ...
    # Check outputs
    if grep -q "risk-score:" action.yml && \
       grep -q "total-findings:" action.yml; then
...
# Test 3: Check Inline PR comments implementation
echo "Test 3: Validating inline PR comment integration..."
if [ -f "packages/core/src/github-commenter.ts" ]; then
    ...
    echo -e "${YELLOW}⚠ github-commenter.ts not found; PR comments are implemented inline in action.yml${NC}"
fi
echo ""
...
if grep -q "github/codeql-action/upload-sarif" action.yml; then
...
if grep -q "sarif_file:" action.yml; then
...
if grep -q "actions/github-script" action.yml; then
...
if grep -q "github.event_name == 'pull_request'" action.yml; then
